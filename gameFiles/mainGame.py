...
while days != 365:
    ...
    if event != 1:
        if event == 4:
            time.sleep(1)
            print("Al llegar a una nueva galaxia, te topaste con una estación. En ella te ofrecen 15 suministros a cambio de 5 de combustible.")
            print("'Y' para aceptar, 'N' para negarte.")
            print("Te quedan "+str(supplies)+" suministros y "+str(Comb)+" de combustible.")
            opc = input()
            if opc == 'Y':
                print("Recibiste 15 suministros a cambio de 5 de combustible.")
                Comb = Comb - 5
            ...
            if UE1 == 0:
                UE1 = 1
                time.sleep(1)
                print("Recibiste una señal de radio de un mecanico, vas a rescatarlo ?")
                print("EFECTOS: Al dormir consumes +1 suministros, nuevos planetas descubiertos, jamas reparar los sistemas de nuevo.")
                print("'Y' para rescatarlo, 'N' para rechazarlo.")
                time.sleep(2)
                print("ADVERTENCIA: Este evento es de una vez, esta elección importa.")
                opc = input()
                if opc != "":
                    if opc == 'Y':
                        print("Muy bien. +1 suministro menos al dormir, jamas reparar sistemas")
                        fixwer = 1
                    elif opc == 'N':
                        print("Muy bien. Nada cambio.")
                    else:
                        event = 1
        elif event == 12:
            if UE1 == 1:
                UE2 = 1
                print("Al navegar por el cosmos encuentras una señal de radio, principalmente estatica, pero algo entendible. Tu mecanico aumento la señal y fueron a por el indviduo.")
                print()
                time.sleep(1)
